...
// Sources Institutionnelles V7 (Légifrance & Bpifrance / Aides Territoires)
const LEGAL_FUNDING_SOURCES = [
  { nom: "Légifrance / JO", q: "(site:legifrance.gouv.fr OR site:service-public.fr) (décret OR loi OR arrêté) (mobilité OR ZFE OR stationnement OR électrique)", tag: "Légal" },
  { nom: "Aides & Subventions", q: "(site:aides-territoires.beta.gouv.fr OR site:bpifrance.fr OR site:francenum.gouv.fr) (subvention OR aide OR appel à projet) (mobilité OR rural OR transition)", tag: "Subventions" }
];
...
const ALL_TARGETS = [...NATIONAL_SOURCES, ...LEGAL_FUNDING_SOURCES, ...REGIONAL_SOURCES];
...
function isArticleValid(titre, desc, sourceNom) {
  ...
  // 5. Filtre Anti-Offres d'Emploi (Ne cible QUE les offres directes, pas les articles sur le recrutement)
  const blacklistEmploi = ["offre d'emploi", "cdi", "cdd", "job dating", "recherche de candidats", "fiche de poste", "nous recrutons"];
  // Note: On retire "recrute", "recrutement", "embauche" pour ne pas bloquer les actus business.
  if (blacklistEmploi.some(w => text.includes(w))) return false;
...
async function generateMarinaInsightFree(titre, desc, sourceNom, region, searchQ) {
  ...
  let resume = `Point de Veille : ${titre.substring(0, 50)}...`;
  let detail = `Note Stratégique : "${premierePhrase}". Événement localisé ${localisation}.`;
  
  let specificProblemQuery = `"${sujetPrincipal}" stratégie OR impact local`;

  let theProblem = `Cet événement impacte potentiellement le tissu économique ou l'organisation territoriale de ${region || 'la région'}.`;
  let theIdea = `📌 *L'analyse d'opportunité IA Premium est en attente (Quota API atteint).* L'article a néanmoins été indexé avec succès dans le Hub Radar pour vos recherches manuelles par mots-clés.`;
...
async function runScout() {
  ...
  for (const source of ALL_TARGETS) {
    ...
    try {
      ...
      const xml = await response.text();
      const articles = parseRSS(xml, 5); // Extraction des 5 articles récents pour couvrir toutes les régions sans saturer l'API !
      ...
      for (const article of articles) {
        ...
        // Si l'IA premium échoue, on garde quand même l'article en DB pour que le moteur de recherche interne puisse le trouver.
        if (!insight_premium && process.env.GEMINI_API_KEY) {
           console.log(`      ⚠️ [Rétrogradation] Quota Gemini API atteint. Injection de l'article en version Essentielle pour la recherche.`);
        }
...
async function startDaemon() {
  await runScout();
  console.log("⏳ Marlowe est en veille active. Prochain scan programmé dans 2 heures...");
  setInterval(runScout, 2 * 60 * 60 * 1000); // Relance automatiquement toutes les 2 heures
}
